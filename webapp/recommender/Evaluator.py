...
class Evaluator:
    """
    Recommender System Evaluator Implementation
    """
    def __init__(self, eval_pos, eval_target, item_popularity, top_k) -> None:
    ...
    def evaluate(self, model, test_batch_size):
        """
        ...
        pred_matrix = model.predict(self.eval_pos, test_batch_size)
    ...
    def gini_diversity(self, topk):
        ...
        item_recommend_counter = np.zeros(num_items, dtype=int)

        rec_item, rec_count = np.unique(topk, return_counts=True)
        item_recommend_counter[rec_item] += rec_count

        item_recommend_counter_mask = np.ones_like(item_recommend_counter, dtype=bool)
